...
def data_attention(subject, eeg = True):
    ...
    eprime      = eprime[['Cond1', 'Cond2', 'Cond3', 'Cond4', 'Image.OnsetTime', 'Image.RESP', 'Image.RT', 'ImageCentre', 'ImageFond', 'ListImage.Cycle']]
    ...
    if eeg:
        ...
        epochs.pick_types(emg=False, eeg=True, stim=False, eog=False, misc=False, exclude='bads')
...
def attention_TF_decoding(subject, freqs=np.arange(3, 30, 1), decim=20):
    ...
    n_cycles = freqs/2
    attention_tfr = tfr_morlet(attention, freqs, n_jobs=6,
...
if __name__ == '__main__':
    ...
    total=[]
    for subject in Names[5:]:
        subject_score = attention_TF_decoding(subject)
        total.append(subject_score)
        np.save(root + 'Results/Attention_TF_decoding/' + subject + '.npy', subject_score)

        plt.rcParams['figure.figsize'] = [10.0, 5.0]
        plt.title('Frequency decoding - ' + subject, fontweight="bold")
        plt.imshow(subject_score,
                   origin = 'lower', aspect = 'auto',vmin = 0.35, vmax=0.65,
...
#    np.save(root + 'Results/Attention_TF_decoding/Total.npy', np.asarray(total))

    
#    # Plot average results
#    scores_total = np.load(root + 'Results/Attention_TF_decoding/Total.npy')
#    
#    plt.rcParams['figure.figsize'] = [10.0, 5.0]
#    max_decod = np.unravel_index(np.asarray(scores_total).mean((0)).argmax(), [27, 101])[1]
#    plt.title('Frequency decoding', fontweight="bold")
#    plt.imshow(np.asarray(scores_total).mean(0), 
#               origin = 'lower', aspect = 'auto',vmin = 0.35, vmax=0.65,
#               cmap=plt.cm.get_cmap('RdBu_r', 20), 
#               interpolation='hanning',
#               extent = [-0.5, 1.5, 3, 30])
#    plt.axvline(x = 0, color = 'k', linewidth = 3)
#    plt.axvline(x = (max_decod/50)-0.5, linestyle='--', color = 'b', linewidth = 3)
#    plt.xlabel('Time (s)')
#    plt.ylabel('Frequencies')
#    plt.colorbar()
#    plt.savefig(root + 'Results/Attention_TF_decoding/frequency_decoding.svg', dpi=300)
#    
#    
#    plt.rcParams['figure.figsize'] = [5.0, 2.0]
#    df = pd.DataFrame(np.asarray(scores_total)[:,:, max_decod]).melt()
#    sns.lineplot(x="variable", y="value", markers=True, data=df,  ci=68, color = 'r')
#    plt.axhline(y=0.5, linestyle='--')
#    plt.savefig(outpath + 'frequency_decoding2.svg', dpi=300)
