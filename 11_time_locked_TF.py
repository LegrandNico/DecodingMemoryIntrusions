    ...
    for ii in range(len(data)):
        ...
        if tnt_df["Black.RESP"].iloc[ii] != 1:
            ...
            for id in indexes:
                ...
                if (id > exclude_peak) & (
                    id < 240
                ):  # Exclude peak < 0.4s & > 2.4s after stim presentation
...
def tfr_permutation(data):
    ...
    plt.title(
        "Time-frequency representation \n time-locked to decoded intrusions",
        size=15,
        fontweight="bold",
    )
...
for i in range(25):
    ...
    for ii in range(102):
        ...
        ch.append(
            trim_mean(this_tfr._data[i, ii][~np.isnan(T_obs_plot)], proportiontocut=0.1)
        )
...
evoked = mne.EvokedArray(T0[:, np.newaxis], this_tfr.average().info, tmin=0.0)
