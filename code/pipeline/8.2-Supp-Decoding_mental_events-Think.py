...
def mental_events(exclude_peak: int):
    ...
    for subject in names:
        ...
        if len(final_df[(final_df.Subject == subject) & (final_df.Condition == "DecodingThink")]) == 0:
            continue
        ...
        time = final_df[(final_df.Subject == subject) & (final_df.Condition == "DecodingThink")].Time.iloc[0]
        length = final_df[(final_df.Subject == subject) & (final_df.Condition == "DecodingThink")].Length.iloc[0]
        ...
        high_CI = np.load(f"{root}Results/Shuffled_95CI/DecodingThink/{subject}-high.npy")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"{root}Results/Decoding/{subject}_DecodingThink_mental-events.png")
        plt.close()

    intrusion_df.to_csv(f"{root}Results/Decoding/DecodingThink_mental_events.txt")
    intrusionTiming_df.to_csv(f"{root}Results/Decoding/DecodingThink_mental_events_timing.txt")
