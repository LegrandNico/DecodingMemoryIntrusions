...
evoked = mne.EvokedArray(
    stats.zscore(np.asarray(gini).mean(0))[:, np.newaxis], attention.info, tmin=0.0
)
    ...
    for subject in Names:
        ...
        for ii in range(len(data)):
            ...
            indexes = peakutils.indexes(
                data[ii, :], thres=0.5, min_dist=5, thres_abs=True
            )
...
def mental_events(exclude_peak):
    ...
    * Matplotlib instance
    ...
    for subject in Names:
        ...
        for block in range(1, 9):
            ...
            for emotion in ["Emotion", "Neutral"]:
                ...
                eeg_intrusions = tnt_df[
                    (tnt_df.Cond2 == emotion) & (tnt_df["ListImage.Cycle"] == block)
                ]["EEG_intrusions"]
    ...
    # Plot averaged subjective vs decoding intrusions
    df = pd.melt(intrusion_df, id_vars=["Block", "Emotions"], value_vars=["Events"])
    plt.rcParams["figure.figsize"] = [8, 6]
    fig, ax = plt.subplots()
        ...
        if dist:
            ...
            for t in range(20, 320, 50):
                ...
                dist_df = dist_df.append(
                    pd.DataFrame(
                        {"Subject": subject, "Time": t, "Percentage": per}, index=[0]
                    ),
                    ignore_index=True,
                )
            ...
            sns.lineplot(
                data=dist_df[dist_df.Subject == subject],
                x="Time",
                y="Percentage",
                ci=68,
                legend="brief",
            ...
            plt.tight_layout()
            plt.savefig(
                root + "Results/Decoding/" + subject + "temporal-distribution.png"
            )
            plt.close()
