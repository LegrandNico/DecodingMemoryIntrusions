...
def mental_events(exclude_peak):
    ...
    * Matplotlib instance
    ...
    for subject in Names:
        ...
        proba = np.load(root + 'Results/Attention_TNT_decoding/'
                        + subject + '_proba.npy')
        labels = np.load(root + 'Results/Attention_TNT_decoding/'
                         + subject + '_labels.npy')
        tnt_df = pd.read_csv(root + 'TNT/Behavior/'
                             + subject + '.txt')
        ...
        high_CI = np.load(root + 'Results/Shuffled_95CI/'
                          + subject + '-high.npy')
